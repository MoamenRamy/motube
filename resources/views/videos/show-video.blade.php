        ...
        <div class="mx-auto col-9">
            ...
            <div class="interaction text-center mt-5">
                ...
                @foreach ($video->views as $view)
                    <span class="float-right">views <span class="viewsNumber">{{$view->views_number}}</span></span>
                @endforeach
            ...
            <div class="mt-4 px-2">
                <div class="comments">
                    ...
                    <div>
                        <textarea class="form-control" id="comment" name="comment" rows="4" placeholder="add comment"></textarea>
                                            ...
                                            <div class="col-10">
                                                @if (Auth::check())
                                                    @if ($comment->user_id == auth()->user()->id || auth()->user()->administration_level > 0)
                                                        @if (!auth()->user()->block)
                                                            <form method="GET" action="{{route('comment.destroy', $comment->id)}}" onsubmit="return confirm('Are you sure you want to delete this comment?')">
                                                                @csrf
                                                                @method('DELETE')
